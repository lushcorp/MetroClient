...
// Creating server.
if (srvname && srvram && srvdisk && srvcpu && srvegg && srvlocation) {


  let packagename = await db.get("package-" + username);
  let usrpackage = config.packages.list[packagename ? packagename : config.packages.default];

  let extra =
    await db.get("extraresources-" + username) ||
    {
      ram: 0,
      disk: 0,
      cpu: 0,
...
if (config.resource_type === "GB") {
  ...
  if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
    ...
    pterodactyl.relationships.servers.data.push(serverinfotext);
    let cputype;
  ...
  return redirect(`/create?error=Missing a Variable`);
}
}
