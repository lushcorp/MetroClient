...
const srvegg = formData.get("egg")?.toString()
  let checkexist = pterodactyl.relationships.servers.data.filter(srves => srves.attributes.id = srvid);
  
  console.log(checkexist, pterodactyl.relationships.servers.data)
  if (checkexist.length !== 1) return new Response(JSON.stringify({
  ...
  if (ram || disk || cpu) {
    ...
    if (ram2 + ram > usrpackage.ram + extraram) return redirect(`/edit?id=${srvid}&error=Exceeding your RAM limit.&limit=${usrpackage.ram + extraram - ram2}`);
    if (disk2 + disk > usrpackage.disk + extradisk) return redirect(`/edit?id=${srvid}&error=Exceeding your Disk limit.&limit=${usrpackage.disk + extradisk - disk2}`);
    if (cpu2 + cpu > usrpackage.cpu + extracpu) return redirect(`/edit?id=${srvid}&error=Exceeding your CPU limit.&limit=${usrpackage.cpu + extracpu - cpu2}`);
